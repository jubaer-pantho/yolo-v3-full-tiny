	...
	for index, x in enumerate(blocks[1:]):
		...
		#If it is a route layer
			...
			if start > 0: 
				start = start - index
			if end > 0:
				end = end - index #setting end to less than zero
			route = EmptyLayer()
			...
			if module_type == "convolutional" or module_type == "upsample" or module_type == "maxpool":
				...
				if len(layers) == 1:
					...
					try:
						...
						mapsize2 = map2.size()
						mapsize1 = map1.size()
						remap1 = torch.zeros(mapsize1[0], mapsize1[1], mapsize2[2], mapsize2[3])
						remap1[:,:,:mapsize1[2],:mapsize1[3]] = map1
						x = torch.cat((remap1, map2), 1)
